...
In-paper anchor: `paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md` (`CBC_G1`).

## Goal
Make the projected response floor explicit as the protected-core gate for `proving persistence of the coarse assembly package through an admissible coarse-geometric closure architecture`.
...
- projected core: the response sector controlled by `kappa_coarse`.
...
`CBC_G1` closes when `kappa_coarse` satisfies the response-floor requirement: projected coarse response has a strict positive floor.
This is the protected-core contribution to the strict margin `M_CBC`.
...
Payload: verify that all quantities used by `kappa_coarse` are defined on the projected sector named in the main preprint.
...
Payload: check the artifact key `kappa_coarse` and the corresponding extraction input/provenance record.

### Theorem EG1.3 (core gate closure)
If Lemmas EG1.1-EG1.2 hold and the runtime artifact accepts `kappa_coarse`, then `CBC_G1` supplies the projected/protected-core input to `M_CBC`.

## Current Instantiation

- gate: `CBC_G1`
- artifact key: `kappa_coarse`
