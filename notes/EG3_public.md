...
In-paper anchor: `paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md` (`CBC_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of the coarse assembly package through an admissible coarse-geometric closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`CBC_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and coarse windows do not collapse.
This is the first-failure capture contribution to `M_CBC`.
...
If near-failure extraction and no-Zeno spacing hold, then `CBC_G3` prevents hidden drift before rigidity is applied.
