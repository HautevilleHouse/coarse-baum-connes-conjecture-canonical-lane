...
In-paper anchor: `paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md` (`CBC_G4`, `CBC_G5`).

## Goal
Separate the rigidity job from the endpoint-transfer job for `proving persistence of the coarse assembly package through an admissible coarse-geometric closure architecture`.
...
- endpoint-transfer lock: `coarse_transfer`.
- coherence interface: `eps_coh` remains available to the bridge and final margin.

## Closure Criterion

`CBC_G4` closes when `rho_rigidity` excludes bad endpoint alternatives: bad nonisomorphic coarse countermodels are excluded.
`CBC_G5` closes when `coarse_transfer` transfers the surviving endpoint to the intended target class: rigid limit transfers to the coarse-assembly endpoint class.
Together they feed the strict margin `M_CBC`.
...
The surviving rigid representative is locked to the standard problem-side endpoint by `coarse_transfer`.

Payload: read this note together with `notes/IDENTIFICATION_BRIDGE.md`.

### Theorem EG4.3 (rigidity/transfer gate closure)
If bad limits are excluded and the endpoint lock is active, then `CBC_G4` and `CBC_G5` deliver the boundary object needed by the final margin.

## Current Instantiation

- rigidity gate: `CBC_G4`
- rigidity artifact key: `rho_rigidity`
- transfer gate: `CBC_G5`
- transfer artifact key: `coarse_transfer`
