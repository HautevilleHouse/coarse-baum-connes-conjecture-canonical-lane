...
In-paper anchor: `paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving persistence of the coarse assembly package through an admissible coarse-geometric closure architecture`.
...
2. the endpoint-transfer lock `coarse_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_CBC` remains positive.
...
Payload: verify `coarse_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_CBC` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `coarse_transfer`
